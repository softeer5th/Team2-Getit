...
import { GetAllRouteRepsonse } from "../api/type/response/route";
...
interface GetAllRouteSSEResponse extends GetAllRouteRepsonse {
	batchSize: number;
}

const BOTTOM_SHEET_HEIGHT = 377;

export default function MapSSEPage() {
	...
	const sseCounter = useRef<number>(0);
		...
		for (const coreRoutes of coreRouteList) {
			...
			const key =
				coreNode1Id < coreNode2Id
					? `SSE_${edges[0].routeId}_${edges.slice(-1)[0].routeId}`
					: `SSE_${edges.slice(-1)[0].routeId}_${edges[0].routeId}`;
	...
	useEffect(() => {
		...
		eventSource.onmessage = (ev) => {
			if (ev.isTrusted) {
				const { batchSize, coreRoutes, nodeInfos }: GetAllRouteSSEResponse = JSON.parse(ev.data);
