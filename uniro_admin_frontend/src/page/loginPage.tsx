    ...
    return (
        <div className='w-screen h-screen flex  items-center justify-center'>
            <div className='w-1/4 h-1/3 flex flex-col border border-dashed border-[#9747FF]'>
                <input className='h-[30px] border border-gray-900 rounded-100 ' type='number' onChange={(e) => setTempId(Number(e.target.value))} value={tempId ?? ''} placeholder='대학교 ID' />
                <input className='h-[30px] border border-gray-900 rounded-100 ' onChange={(e) => setCode(e.target.value)} value={code} placeholder='입장 코드' />
