...
const BuildingPage = () => {
	...
	const [selectedCoord, setSelectedCoord] = useState<Coord | undefined>(undefined);
	...
	const [buildingMarkers, setBuildingMarkers] = useState<google.maps.marker.AdvancedMarkerElement[]>([]);
	...
	const drawRoute = (coreRouteList: CoreRoutesList, mode: "add" | "connect" | "view") => {
	...
	const addBuildings = () => {
		if (google.maps.marker.AdvancedMarkerElement === null || map === null) return;
	...
	return (
		<MainContainer>
			...
			<BuildingAddContainer
				...
				mode={mode}
				selectedBuilding={buildings.data?.find((building) => building.nodeId === selectedBuildingId) || null}
			/>
		</MainContainer>
	);
};
