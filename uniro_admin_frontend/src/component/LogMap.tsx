...
export default function LogMap({ center, revisionData, setInfo }: LogMapProps) {
    ...
    const { cautionMarkerElement, dangerMarkerElement, changedMarkerElement, removedMarkerElement, createdMarkerElement } = createMarkerElement();
    ...
    const addRiskMarkers = (type: Markers.DANGER | Markers.CAUTION, routes: DangerRoute[] | CautionRoute[]) => {
        ...
        if (type === Markers.DANGER) {
            (routes as DangerRoute[]).forEach((route) => {
                ...
                const removeMarker = createAdvancedMarker(
                    AdvancedMarker,
                    null,
                    centerCoordinate(route.node1, route.node2),
                ...
                cachedMarker.current.set(routeId, { DEFAULT: dangerMarker, REMOVED: removeMarker });
            })
            return;
        }
        ...
        if (type === Markers.CAUTION) {
            (routes as CautionRoute[]).forEach((route) => {
                ...
                cachedMarker.current.set(routeId, { DEFAULT: cautionMarker, REMOVED: removeMarker });
            })
            return;
        }
    }
