...
const BuildingAddContainer: React.FC<BuildingAddContainerProps> = ({
	...
	const addBuildingRoute = useMutation({
		mutationFn: (body: { buildingNodeId: NodeId; nodeId: NodeId }) => postBuildingRoute(university?.id ?? -1, body),
		onSuccess: () => {
			alert("경로가 추가되었습니다.");
			...
			{mode === "add" || mode === "view" ? (
				selectedCoord || selectedBuilding ? (
					<>
						...
						<form className="flex-1 flex flex-col items-start w-full h-full p-4" onSubmit={handleSubmit}>
				...
				<div className="flex flex-col justify-start w-full h-full text-left p-4 bg-white rounded-lg shadow-md border border-gray-300">
					<h3 className="text-lg font-semibold text-gray-700 mb-3">선택된 정보</h3>
					<button
                ...
                bg-blue-500 text-white hover:bg-blue-600 mb-2"
					...
					{selectedBuilding ? (
						<div className="p-3 bg-gray-100 rounded-md shadow-sm mb-2">
							<p className="text-sm text-gray-900">
								🏢 <span className="font-medium">건물명:</span> {selectedBuilding.buildingName}
							</p>
						</div>
					) : (
					...
					{selectedNode.length > 0 ? (
						<>
							...
							>
								<p className="text-sm text-gray-600 font-medium">📍 선택한 노드 정보 1</p>
								<p className="text-sm text-blue-500">NODE ID: {selectedNode[0].nodeId}</p>
								<p className="text-sm text-blue-500">위도: {selectedNode[0].lat}</p>
								<p className="text-sm text-green-500">경도: {selectedNode[0].lng}</p>
								<div className="flex justify-end w-full">
									<button
										onClick={() => {
											drawSingleRoute(
                ...
                bg-blue-500 text-white hover:bg-blue-600"
							...
							<div
								className={`flex flex-col p-3 bg-gray-100 rounded-md shadow-sm ${!isSelectedNode1 && "border-2 border-blue-500"} mb-2`}
							>
								<p className="text-sm text-gray-600 font-medium">📍 선택한 노드 정보 2</p>
								<p className="text-sm text-blue-500">NODE ID: {selectedNode[1].nodeId}</p>
								<p className="text-sm text-blue-500">위도: {selectedNode[1].lat}</p>
								<p className="text-sm text-green-500">경도: {selectedNode[1].lng}</p>
								<div className="flex justify-end w-full">
									<button
										onClick={() => {
											drawSingleRoute(
                ...
                bg-red-500 text-white hover:bg-red-600"
						...
						<p className="text-sm text-gray-500">📍 선택된 길 좌표가 없습니다.</p>
					)}
				</div>
			)}
		</div>
